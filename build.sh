...
_warn "Adding kiosk specific configs"
cp -r $SRC_DIR/flavors/kiosk/kiosk-initial-setup	$CHROOT_DIR/usr/lib/shopno-os/						# kiosk initial setup tool
cp -r $SRC_DIR/flavors/kiosk/skel/openbox	$CHROOT_DIR/etc/skel/.config/	# openbox configuration	
cp -r $SRC_DIR/flavors/kiosk/skel/touchegg	$CHROOT_DIR/etc/skel/.config/	# touchegg configuration
cp -r $SRC_DIR/flavors/kiosk/skel/xfce4     $CHROOT_DIR/etc/skel/.config/   # kiosk specific xfce4 setttings
cp -r $SRC_DIR/flavors/kiosk/onboard        $CHROOT_DIR/etc/skel/.local/share/  # onboard themes and settings
cp    $SRC_DIR/flavors/kiosk/start-kiosk-setup 	$CHROOT_DIR/usr/local/bin/	# 
...
cp -r $SRC_DIR/icons/*          $CHROOT_DIR/usr/share/icons/		# icons
cp -r $SRC_DIR/themes/*    $CHROOT_DIR/usr/share/themes/	        # themes

# Drivers:
# RTL8821CE
# cp -r $SRC_DIR/drivers/*        $CHROOT_DIR/usr/src/        		# Wlan driver
cp -r $SRC_DIR/src/onboard            $CHROOT_DIR/usr/src/        		# Wlan driver
...
echo "unclutter zenity openbox libpugixml1v5 firefox-esr" > $LB_CONFIG_DIR/package-lists/kiosk.list.chroot
